...
void main() {
  ...
  group('PaymentsApi.update', () {
    test('only appends ?email_receipt=true when sendEmail is true (matches '
        'admin-portal payment_repository.dart:87-89)', () async {
      http.BaseRequest? captured;
      final fake = MockClient((req) async {
        captured = req;
        return _envelope();
      ...
      await PaymentsApi(_client(fake)).update(
        id: 'p_1',
        payload: {'number': 'P-002', kPaymentSendEmailKey: true},
        idempotencyKey: 'idem',
      );
    ...
    test(
      ...
      () async {
        ...
        expect(
          captured!.url.queryParameters.containsKey('email_receipt'),
          isFalse,
        );
      },
    );
  ...
  group('PaymentsApi.refund', () {
    test('POSTs /payments/refund with body {id, date, invoices} and lifts '
        'sendEmail/gatewayRefund to query params', () async {
      http.BaseRequest? captured;
      final fake = MockClient((req) async {
        captured = req;
        return _envelope();
    ...
    test(
      ...
      () async {
        ...
        expect(captured!.url.queryParameters['email_receipt'], 'false');
        expect(
          captured!.url.queryParameters.containsKey('gateway_refund'),
          isFalse,
        );
      },
    );
  ...
  group('PaymentsApi.apply', () {
    test(
      ...
      () async {
        ...
        final allocations = (body['invoices'] as List)
