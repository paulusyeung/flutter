...
void main() {
  testWidgets(
    'renders one row per item where show != false',
    (tester) async {
  ...
  testWidgets(
    'items with show:false are filtered out',
    (tester) async {
      final spec = blockSpecFor('total')!;
      final hiddenItems = [
      ...
      await tester.pumpWidget(_wrap(
        TotalBlock(block: block, sample: DesignerSampleData.fallback),
      ));
      await tester.pump();
  ...
  testWidgets('isTotal and isBalance items render in bold weight', (tester) async {
    await tester.pumpWidget(_wrap(
      TotalBlock(
        block: _totalBlock(),
        sample: DesignerSampleData.fallback,
