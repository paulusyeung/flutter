...
Widget _wrap(Widget child) => MaterialApp(
  ...
  home: Scaffold(body: ReorderableListView(
    onReorder: (_, _) {},
    children: [
      Container(key: const ValueKey('only-row'), child: child),
    ],
  )),
);
...
void main() {
  group('ExpandablePropertyRow (Phase 15b)', () {
    testWidgets('collapsed → expandedChild is not mounted', (tester) async {
      await tester.pumpWidget(_wrap(ExpandablePropertyRow(
        index: 0,
    ...
    testWidgets('tapping the chevron fires onToggleExpanded',
        (tester) async {
      var toggles = 0;
      await tester.pumpWidget(_wrap(ExpandablePropertyRow(
        index: 0,
