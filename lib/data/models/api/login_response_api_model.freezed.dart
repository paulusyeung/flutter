...
@JsonKey(name: 'subdomain') String get subdomain;@JsonKey(name: 'portal_domain') String get portalDomain;@JsonKey(name: 'portal_mode') String get portalMode;@JsonKey(name: 'client_registration_fields') List<ClientRegistrationFieldApi> get clientRegistrationFields;@JsonKey(name: 'custom_fields') Map<String, String> get customFields;// Company file attachments. The server ships these on the login/refresh
...
// offline and before its own `GET /companies/{id}` lands. Without this the
// `_persistAndActivate` wipe+upsert nulls the column on every refresh.
@override@JsonKey(name: 'documents') List<DocumentApi> get documents {
...
@JsonKey(includeFromJson: false, includeToJson: false)
@override
int get hashCode => Object.hashAll([runtimeType,id,displayName,name,companyKey,updatedAt,subdomain,portalDomain,portalMode,const DeepCollectionEquality().hash(_clientRegistrationFields),const DeepCollectionEquality().hash(_customFields),const DeepCollectionEquality().hash(_documents),sizeId,industryId,firstMonthOfYear,firstDayOfWeek,useCommaAsDecimalPlace,legalEntityId,enabledModules,const DeepCollectionEquality().hash(_settings),const DeepCollectionEquality().hash(_taskStatuses),const DeepCollectionEquality().hash(_companyGateways),const DeepCollectionEquality().hash(_paymentTerms),const DeepCollectionEquality().hash(_taxRates),const DeepCollectionEquality().hash(_expenseCategories),const DeepCollectionEquality().hash(_groups),const DeepCollectionEquality().hash(_bankTransactionRules),const DeepCollectionEquality().hash(_bankIntegrations),const DeepCollectionEquality().hash(_webhooks),const DeepCollectionEquality().hash(_tokensHashed),const DeepCollectionEquality().hash(_taskSchedulers),const DeepCollectionEquality().hash(_subscriptions),const DeepCollectionEquality().hash(_designs),enabledTaxRates,enabledItemTaxRates,enabledExpenseTaxRates,calculateTaxes,taxData,customSurchargeTaxes1,customSurchargeTaxes2,customSurchargeTaxes3,customSurchargeTaxes4,trackInventory,stockNotification,inventoryNotificationThreshold,enableProductDiscount,enableProductCost,enableProductQuantity,defaultQuantity,showProductDetails,fillProducts,updateProducts,convertProducts,convertRateToClient,stopOnUnpaidRecurring,useQuoteTermsOnConversion,googleAnalyticsKey,matomoId,matomoUrl,sessionTimeout,defaultPasswordTimeout,oauthPasswordRequired,isDisabled,markdownEnabled,markdownEmailEnabled,reportIncludeDrafts,reportIncludeDeleted,const DeepCollectionEquality().hash(_quickbooks)]);
