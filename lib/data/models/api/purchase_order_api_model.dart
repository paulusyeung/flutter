...
part 'purchase_order_api_model.g.dart';

/// Raw JSON shape of `/api/v1/purchase_orders/{id}`. Same field set as
/// [QuoteApi] but vendor-centric: `vendor_id` is primary, `client_id`
/// is empty, and `expense_id` carries the receipt → expense linkage.
@freezed
abstract class PurchaseOrderApi with _$PurchaseOrderApi {
  const factory PurchaseOrderApi({
    @Default('') String id,
  ...
  factory PurchaseOrderApi.fromJson(Map<String, dynamic> json) =>
      _$PurchaseOrderApiFromJson(json);
}
...
abstract class PurchaseOrderListApi with _$PurchaseOrderListApi {
  ...
  factory PurchaseOrderListApi.fromJson(Map<String, dynamic> json) =>
      _$PurchaseOrderListApiFromJson(json);
}

@freezed
abstract class PurchaseOrderItemApi with _$PurchaseOrderItemApi {
  const factory PurchaseOrderItemApi({required PurchaseOrderApi data}) =
      _PurchaseOrderItemApi;
