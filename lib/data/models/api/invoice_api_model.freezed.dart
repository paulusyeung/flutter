...
@JsonKey(name: 'e_invoice') Map<String, dynamic>? get eInvoice;@JsonKey(name: 'backup') Map<String, dynamic>? get backup;@JsonKey(name: 'tax_info') Map<String, dynamic>? get taxInfo;// Rectification (Verifactu "factura rectificativa"): id of the original
// invoice this one corrects + the user-supplied rectification reason.
@JsonKey(name: 'modified_invoice_id') String? get modifiedInvoiceId;@JsonKey(name: 'reason') String? get reason;// Read-only payment-schedule projection. Present ONLY when the invoice
...
@JsonKey(name: 'schedule') List<ScheduleItemApi>? get schedule;// Flags
...
as List<ScheduleItemApi>?,isLocked: null == isLocked ? _self.isLocked : isLocked // ignore: cast_nullable_to_non_nullable
...
// invoice this one corrects + the user-supplied rectification reason.
@override@JsonKey(name: 'modified_invoice_id') final  String? modifiedInvoiceId;
...
// stored column" from "present → authoritative". Persisted to a
// dedicated Drift column; `Invoice.toApiJson` omits it (read-only).
 final  List<ScheduleItemApi>? _schedule;
// Read-only payment-schedule projection. Present ONLY when the invoice
// is fetched with `?show_schedule=true` (probe-verified: absent on
// plain/list GETs). Nullable for the same reason as `documents` — the
// repo's `_apiToCompanion` must distinguish "key absent → preserve the
// stored column" from "present → authoritative". Persisted to a
// dedicated Drift column; `Invoice.toApiJson` omits it (read-only).
@override@JsonKey(name: 'schedule') List<ScheduleItemApi>? get schedule {
...
@JsonKey(includeFromJson: false, includeToJson: false)
@override
int get hashCode => Object.hashAll([runtimeType,id,number,poNumber,date,dueDate,partialDueDate,statusId,clientId,vendorId,projectId,designId,assignedUserId,userId,locationId,subscriptionId,parentInvoiceId,recurringId,const DeepCollectionEquality().hash(amount),const DeepCollectionEquality().hash(balance),const DeepCollectionEquality().hash(paidToDate),const DeepCollectionEquality().hash(totalTaxes),const DeepCollectionEquality().hash(discount),const DeepCollectionEquality().hash(partial),isAmountDiscount,const DeepCollectionEquality().hash(exchangeRate),taxName1,taxName2,taxName3,const DeepCollectionEquality().hash(taxRate1),const DeepCollectionEquality().hash(taxRate2),const DeepCollectionEquality().hash(taxRate3),usesInclusiveTaxes,const DeepCollectionEquality().hash(customSurcharge1),const DeepCollectionEquality().hash(customSurcharge2),const DeepCollectionEquality().hash(customSurcharge3),const DeepCollectionEquality().hash(customSurcharge4),customTaxes1,customTaxes2,customTaxes3,customTaxes4,publicNotes,privateNotes,terms,footer,customValue1,customValue2,customValue3,customValue4,const DeepCollectionEquality().hash(_lineItems),const DeepCollectionEquality().hash(_invitations),const DeepCollectionEquality().hash(_documents),reminder1Sent,reminder2Sent,reminder3Sent,reminderLastSent,reminderSchedule,frequencyId,nextSendDate,nextSendDatetime,lastSentDate,remainingCycles,dueDateDays,autoBill,autoBillEnabled,const DeepCollectionEquality().hash(_eInvoice),const DeepCollectionEquality().hash(_backup),const DeepCollectionEquality().hash(_taxInfo),modifiedInvoiceId,reason,const DeepCollectionEquality().hash(_schedule),isLocked,isDeleted,createdAt,updatedAt,archivedAt]);
