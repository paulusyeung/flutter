...
const kCustomFieldsUsersSearchKeys = <String>[
  'user_field',
  'label',
  'field_type',
  'single_line_text',
...
class CustomFieldsUsersScreen extends StatelessWidget {
  const CustomFieldsUsersScreen({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return SettingsFormShell(
      sections: [
        FormSection(
          title: context.tr('user_field'),
          children: [
            for (var i = 1; i <= 4; i++)
              CustomFieldRow<CustomFieldsViewModel>(
                key: ValueKey('${access.companyId}:user$i'),
                prefix: 'user',
                slot: i,
