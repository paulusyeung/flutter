...
class ProjectEditScreen extends StatelessWidget {
  const ProjectEditScreen({this.existingId, this.cloneFrom, super.key});

  final String? existingId;

  /// Edit-mode override draft. Null for a normal edit and for create (which
  /// reads the staged draft via `Services.takeCreateDraft`).
    ...
    return EntityEditScreenScaffold<Project, ProjectEditViewModel>(
      ...
      buildVm: (ctx, services, companyId, existing) {
        ...
        final clone =
            cloneFrom ??
            (existing == null
                ? services.takeCreateDraft<Project>('/projects')
                : null);
