...
- `NativeWindow` (`lib/app/native_window.dart`) — bridge over the `invoice_ninja/native_window` method channel, mirroring the `native_window_theme` bridge; no-op off desktop. Method set: `startDrag`, `doubleClick`, `minimize`, `toggleMaximize`, `close`.
...
**Channel method set** — `startDrag` (begin a native move-drag), `doubleClick` (title-bar double-click action), `minimize`, `toggleMaximize`, `close`. macOS implements only `startDrag` + `doubleClick` (its native buttons cover the rest); Windows/Linux implement all five.
