...
**Don't run integration tests locally unless the user explicitly asks** — they take over the foreground app and interrupt the developer's session. Never run them proactively or as incidental verification. When the user explicitly asks, run them in an isolated worktree (see **Running locally on request** below). CI runs them via `.github/workflows/ci.yaml` on manual `workflow_dispatch`: the `apple` job runs the full suite **per file** on **macOS desktop** via `tools/run_integration_local.sh --include-demo` (incl. the live `demo/*` files when reachable), and the `integration-web` job runs `app_smoke_test.dart` on **Chrome** (`-d web-server --browser-name=chrome`) to guard the web build. The live `demo/*` suite and any filesystem/secure-storage/`pruneBrokenDbFiles` unit tests are `@TestOn('vm')` / macOS-only — they don't run under the web job.
